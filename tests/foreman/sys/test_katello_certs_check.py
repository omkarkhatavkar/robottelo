...
class TestKatelloCertsCheck:
    ...
    def cert_setup_destructive_teardown(self, destructive_sat):
        ...
        sat6_hostname = destructive_sat.hostname
    ...
    def test_positive_update_katello_certs(self, cert_setup_destructive_teardown):
        ...
        try:
            ...
            assert result.status == 0, 'Hammer Ping fail'
            command = (
    ...
    def test_positive_generate_capsule_certs_using_relative_path(
        ...
        satellite.execute('mkdir -p /root/capsule_cert')
        satellite.execute(
            'cp "{}" /root/capsule_cert/ca_cert_bundle.pem'.format(cert_data['ca_bundle_file_name'])
        )
        satellite.execute(
...
class TestCapsuleCertsCheckTestCase:
    ...
    def capsule_certs_teardown(self, default_sat):
        ...
        default_sat.capsule_certs_generate(capsule, certs_path=f'{tmp_dir}/capsule_certs.tar')
        return {
            'tmp_dir': tmp_dir,
